...
h1{margin:0;font-size:30px}.muted{color:#9ca8b5}.badge{border:1px solid #31404d;border-radius:999px;padding:7px 11px;font-size:12px}
.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.card{background:#12161b;border:1px solid #252d35;border-radius:14px;padding:16px}
.metric{font-size:26px;font-weight:700;margin-top:8px}.wide{grid-column:span 2}h2{font-size:16px;margin:0 0 12px}
table{width:100%;border-collapse:collapse;font-size:13px}th,td{text-align:left;padding:9px 7px;border-bottom:1px solid #252d35}th{color:#9ca8b5;font-weight:500}
button{background:#eef2f6;color:#111;border:0;border-radius:9px;padding:9px 12px;font-weight:700;cursor:pointer}
pre{white-space:pre-wrap;word-break:break-word;background:#0b0d10;padding:12px;border-radius:9px;max-height:260px;overflow:auto}
section{margin-top:12px}.ok{color:#8ee6a5}.warn{color:#ffd580}@media(max-width:850px){.grid{grid-template-columns:1fr 1fr}.wide{grid-column:span 2}}@media(max-width:520px){.grid{grid-template-columns:1fr}.wide{grid-column:span 1}header{align-items:start;flex-direction:column}}
</style>
</head>
<body><main>
<header><div><div class="muted">Research & paper execution control plane</div><h1>Trading Platform</h1></div><div class="badge"><span class="ok">●</span> PAPER ONLY · LIVE OFF</div></header>
<div class="grid">
...
<div class="card"><div class="muted">Audit events</div><div id="events" class="metric">—</div></div>
...
async function refresh(){
 try{
  const [m,d,e,p,a]=await Promise.all([j('/metrics'),j('/datasets'),j('/simulations/engines'),j('/paper/portfolio'),j('/paper/audit?limit=10')]);
  ...
  positions.textContent=m.open_positions;events.textContent=m.audit_events;
